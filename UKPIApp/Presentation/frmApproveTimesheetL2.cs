...
{
    public partial class FrmApproveTimesheetL2 : Form
    {
        #region Private fields

        private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(FrmApproveTimesheetL2));
        ...
        {
            DataTable tbL1 = _ccllv.GetTruongNhomL1(clsSystemConfig.UserName);

            cboTruongNhomL1.ValueMember = "USERNAME";
            cboTruongNhomL1.DisplayMember = "QLL1";
            cboTruongNhomL1.DataSource = tbL1;
            //txtTeamlead.Text = clsSystemConfig.FullName;
        ...
        private void OnSearchLichLamViec()
        {
            try
            {
                ...
                var table = lichLamViec.GetChamCongLichLamViecL2(week, fromDate, toDate, teamLead, onOff, shift, status,
                    team);
        ...
        private void grdStore_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                var dialogResult = MessageBox.Show(this.ValidatedDateToSave(), clsResources.GetMessage("Messages.FrmApproveTimesheet.Approve"), MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
                switch (dialogResult)
                {
        ...
        /// <summary>
        /// grdStore_RowHeadersWidthChanged
        /// Creator: Sonlv
        /// Create Date: 26/04/2010
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void grdStore_RowHeadersWidthChanged(object sender, EventArgs e)
        {
            //clsCommon.ReLocateSelectAllCheckbox(grdStores.RowHeadersWidth, grdStores.Left, grdStores.Top, chkDelete);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void grdStore_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            //if (grdStores.CurrentRow != null && grdStores.CurrentCell != grdStores.CurrentRow.Cells[FieldCheck]) return;
            //var dt = grdStores.DataSource as System.Data.DataTable;
            //var rows = GetRowSelected(dt, FieldCheck, ValueTrue);
            //_checkRowsCount = rows.Length;
            ////check enable of button btDelete
            //btnExport.Enabled = _checkRowsCount > 0;
        }
        ...
        public void SendEmailForL3()
        {
            try
            {
                string tenTruongNhom = clsSystemConfig.FullName;
                DataTable tb = _lichLamViecBo.GetEmailOfLeverApprove(Int32.Parse(clsSystemConfig.MaNhanVien.ToString()), 2);
                string emailL3 = tb.Rows[0]["Email"].ToString();
                string tenNVQLL3 = tb.Rows[0]["FullName"].ToString();

                string body = "";

                body = "<p>Chào " + tenNVQLL3 + " </p></br></br></br> ";
                ...
                _common.SendEmail("", emailL3, "Email Thông báo phê duyệt Chấm công.", body);
            }
            catch (Exception ex)
            {
                Log.Error(ex.Message, ex);
        ...
        public List<ClsLichLamViec> GetDataToApproveSave()
        {
            ...
            lstLichLamViec.AddRange(from DataRow row in table.Rows
                                    ...
                                    select new ClsLichLamViec
                                    {
                                        SysId = long.Parse(row[ChamCongLichLamViecDAO.SysID].ToString()),
                                        L2XacNhan = true,
                                        L2XacNhan_Date = DateTime.Now.ToString(clsCommon.ApproveTimesheet.DateFormatDb),
                                        L2XacNhan_Id = clsSystemConfig.MaNhanVien.ToString(),
                                        L2XacNhan_TenNgan = clsSystemConfig.FullName,
                                        LastUpDate = DateTime.Now.ToString(clsCommon.ApproveTimesheet.DateFormatDb),
                                        lastUpdateId = clsSystemConfig.MaNhanVien.ToString(),
                                        Note = row[ChamCongLichLamViecDAO.Note].ToString()
                                    });
        ...
        private bool OnApprove()
        {
            try
            {
                ...
                lichLamViec.XacNhanChamCongLichLamViecL2(lstChamCong);
        ...
        private void btnReject_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(clsResources.GetMessage("messages.FrmApproveTimesheet.NoDataFoundToRejected"),
                              clsResources.GetMessage("messages.general"), MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                var dialogResult = MessageBox.Show(clsResources.GetMessage("messages.FrmApproveTimesheet.ConfirmToRejected") + Environment.NewLine
                                                        + clsResources.GetMessage("messages.FrmApproveTimesheet.WantToRejectedTS"), clsResources.GetMessage("Messages.frmApproveTimesheet.ApproveTS"), MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
                switch (dialogResult)
                {
                    case DialogResult.Cancel:
                        // e.Cancel = true;
